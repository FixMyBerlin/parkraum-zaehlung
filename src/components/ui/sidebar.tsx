...
export function SidebarBody({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
    <div
      ...
      className={clsx(
        className,
        'flex min-h-0 flex-1 flex-col overflow-y-auto p-4 [&>[data-slot=section]+[data-slot=section]]:mt-8',
      )}
    />
  )
}
  ...
  return (
    <span className={clsx(className, 'relative')}>
      ...
      {typeof props.href === 'string' ? (
        ...
        <Headless.Button
          {...props}
          className={clsx('cursor-pointer data-disabled:cursor-not-allowed', classes)}
          data-current={current ? 'true' : undefined}
          ref={ref}
        >
          <TouchTarget>{children}</TouchTarget>
